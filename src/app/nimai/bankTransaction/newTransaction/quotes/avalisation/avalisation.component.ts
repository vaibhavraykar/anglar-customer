...
export class AvalisationComponent implements OnInit {
 ...
 this.tab='tab1';
  ...
  public transaction(act: string) {
    ...
    switch (act) {
      ...
      case 'submit': {
        ...
        break;
        case 'withdraw': {          
          $('#withdrawTrasactionvalisation').show();
          // const param={
          //   "transactionId":this.dataViewEdit.transactionId,
          ...
          case 'withdrawTransaction': {
            ...
            this.ts.withdrawQuote(param).subscribe(
              (response) => {
                    $('#withdrawTrasactionvalisation').hide();
                this.tab = 'tab4';
              },
              error => {
                alert('error')
  ...
  closed_div(){
    ...
    document.getElementById("menubarAvalisationQuote").style.width="0%";
    document.getElementById("myModalConfirm").style.width = "0";
    document.getElementById("validateSuccessCon").style.width = "0";
    // document.getElementById("withdrawTrasactionConfirm").style.width = "0";
      ...
      case 'calculateQuote': {        
        this.ts.saveQuotationToDraft(this.data, this.ID).subscribe(
          (response) => {
            ...
            console.log(this.data.TotalQuote);
          },
          error => {
            alert('error')
            this.closedQuote();
