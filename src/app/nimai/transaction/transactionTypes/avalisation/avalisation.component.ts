...
export class AvalisationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.date = new Date();
    this.todayDate =formatDate(this.date.setDate( this.date.getDate() + 1 ),'yyyy-MM-dd', 'en');
...
this.checknewdate = this.todayDate;
 ...
 onItemSelect(item) {
  ...
  closed_div(){
    ...
    document.getElementById('menu-bar-av').style.width = "0";
    // document.getElementById('cancelTrasactionBG').style.width = "0";
    // document.getElementById("cancelTrasactionBG").style.opacity = "0"; 
        ...
        this.imgDownload=false;
          ...
          // }        
            ...
            calculateDiff() {
              ...
              this.data.discountingPeriod = (diffInDays +1);
            }
            newdate(val){
              var firstdate = formatDate((new Date(this.data.lCIssuingDate) ),'yyyy-MM-dd', 'en');
              console.log(firstdate);
              if (firstdate > val) {
                console.log(' im small')
                this.checkdate = firstdate; 
